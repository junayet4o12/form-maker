    ...
    return (
        <div className={`transition-all duration-500 w-full`}>
            ...
            <div className="flex flex-col gap-5">
                ...
                <div className=" max-w-[500px] mx-auto flex justify-center items-center gap-10">
                    <button onClick={handleUpdate} className={`btn w-full btn-primary bg-secondary/80 border-none hover:bg-secondary  my-5  ${user?.email === formDetails.userEmail ? `scale-100` : 'scale-x-0'} `}>Update</button>
                    <button onClick={handleDelete} className={`btn w-full btn-primary bg-primary/80 border-none hover:bg-primary my-5  ${user?.email === formDetails.userEmail ? `scale-100` : 'scale-x-0'} `}>Delete</button>
                </div>
            </div>
        </div>
    );
